...
USE_CLOUDINARY_STORAGE = getattr(settings, 'USE_CLOUDINARY_STORAGE', False)

if USE_CLOUDINARY_STORAGE:
    from cloudinary.models import CloudinaryField as StorageField
else:
    StorageField = models.FileField
...
class CandidateTable(models.Model):
    ...
    if USE_CLOUDINARY_STORAGE:
        candidate_cv = StorageField('resume', blank=True, null=True)  # For storing CV/resume files
        profile_image = StorageField('profile_image', blank=True, null=True)  # For storing profile images
    else:
        candidate_cv = StorageField(upload_to='candidates/cv/', blank=True, null=True)
        profile_image = StorageField(upload_to='profile_images/', blank=True, null=True)
    ...
    if USE_CLOUDINARY_STORAGE:
        resume = StorageField('resume', blank=True, null=True)
    else:
        resume = StorageField(upload_to='resumes/', blank=True, null=True)
...
class RecruiterTable(models.Model):
    ...
    if USE_CLOUDINARY_STORAGE:
        ...
        profile_image = StorageField(upload_to='profile_images/', blank=True, null=True)  # For storing profile images
